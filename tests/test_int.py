...
def test_aws_service_not_running_without_cert(nhs_login_mock_token):
    headers = {
        "Authorization": f"Bearer {nhs_login_mock_token}",
        "X-Request-ID": "60E0B220-8136-4CA5-AE46-1D97EF59D068",
    }
    service_domain_name = os.getenv('AWS_DOMAIN_NAME')
    with pytest.raises(requests.exceptions.RequestException) as excinfo:
        requests.get(f"{service_domain_name}/status", headers=headers)
