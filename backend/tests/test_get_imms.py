...
class TestGetImmunisationById(unittest.TestCase):
    ...
    def test_get_immunization_by_id_with_header(self):
        """it should return Immunization by id"""
        lambda_event = {'headers': {'identifierSystem': 'an-id'},"pathParameters": {"id": "an-id"}}
        exp_res = {"a-key": "a-value"}
    ...
    def test_handle_exception_with_header(self):
        """unhandled exceptions should result in 500"""
        lambda_event = {'headers': {'identifierSystem': 'an-id'},"pathParameters": {"id": "an-id"}}
        error_msg = "an unhandled error"
        self.controller.get_immunization_by_identifier.side_effect = Exception(error_msg)
