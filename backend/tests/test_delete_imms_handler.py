...
class TestDeleteImmunizationById(unittest.TestCase):
    ...
    @patch("delete_imms_handler.controller", autospec=FhirController)
    def test_delete_immunization(self, mock_controller):
        """it should delete the given Immunization by id"""
        lambda_event = {"pathParameters": {"id": "an-id"}}
        exp_res = {"a-key": "a-value"}

        mock_controller.delete_immunization.return_value = exp_res

        # When
        act_res = delete_immunization(lambda_event)

        # Then
        mock_controller.delete_immunization.assert_called_once_with(lambda_event)
        self.assertDictEqual(exp_res, act_res)

    @patch("delete_imms_handler.uuid", autospec=uuid)
    @patch("delete_imms_handler.controller", autospec=FhirController)
    def test_handle_exception(self, mock_controller, mock_uuid):
        ...
        mock_controller.delete_immunization.side_effect = Exception(error_msg)
