...
class PreValidation:
    ...
    def for_decimal(
        ...
        """
        Apply pre-validation to a decimal field to ensure that it is a decimal
        """
        if not type(field_value) is Decimal:  # pylint: disable=unidiomatic-typecheck
            raise TypeError(f"{field_location} must be a decimal")
