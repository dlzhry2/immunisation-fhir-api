...
class PostValidators:
    ...
    def __init__(self, imms):
        ...
        self.status = self.imms.get("status")
        # Note that the majority of fields require standard validation. Exceptions not included in this list are
        # vaccine_type, reason_code_coding_code and reason_code_coding_field
    ...
    def obtain_field_location(self, field_name):
        """Returns the field location string"""
        return getattr(FieldLocations, field_name)

    def obtain_field_value(self, field_name):
        ...
        try:
            field_value = function_for_obtaining_field_value(self.imms)
        except (KeyError, IndexError):
            field_value = None

        return field_value

    def run_field_validation(self, validation_set, field_name, field_location, field_value):
        """Ascertains the correct validation rule from the given validation set and applies the validation"""

        # Obtain the relevant validation function for this field
        validation_function = validation_set[field_name]

        # Run the validation function
        try:
            validation_function(
                imms=self.imms, field_value=field_value, field_location=field_location, vaccine_type=self.vaccine_type
            )
    ...
    def validate_field(self, validation_set, field_name):
        """Runs standard validation for the field"""

        field_location = self.obtain_field_location(field_name)
        field_value = self.obtain_field_value(field_name)
        self.run_field_validation(validation_set, field_name, field_location, field_value)

    def validate_reason_code_coding_field(self, validation_set, field_type):
        """
    ...
    def validate(self):
        ...
        # Identify the relevant validation set based on the vaccine type
        ...
        for field_name in self.fields_with_standard_validation:
            self.validate_field(field_name=field_name, validation_set=validation_set)

        # Validation reason_code_coding fields. Note that there may be multiple of each of these
        # - all instances of the field will be validated by the validate_reason_code_coding_field validator
        self.validate_reason_code_coding_field(validation_set, "code")
        self.validate_reason_code_coding_field(validation_set, "display")
