...
class TestAuthenticator(unittest.TestCase):
    def setUp(self):
        ...
        env = "an-env"
        self.authenticator = AppRestrictedAuth(Service.MNS, self.secret_manager_client, env, self.cache)
    ...
    def test_env_mapping(self):
        ...
        # For env=none-prod
        env = "some-env"
        auth = AppRestrictedAuth(Service.MNS, None, env, None)
    ...
    def test_update_cache(self):
        ...
        # Then
        self.cache.put.assert_called_once_with(f"{Service.MNS.value}_access_token", cached_token)
