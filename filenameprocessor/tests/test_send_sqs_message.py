...
class TestSendSQSMessage(TestCase):
    ...
    def test_send_to_supplier_queue_success(self):
        ...
        self.assertIsNone(send_to_supplier_queue(deepcopy(MockFileDetails.flu_emis.sqs_message_body)))
        ...
        self.assertEqual(json_loads(messages["Messages"][0]["Body"]), MockFileDetails.flu_emis.sqs_message_body)
    ...
    def test_send_to_supplier_queue_failure_due_to_queue_does_not_exist(self):
        ...
        with self.assertRaises(UnhandledSqsError) as context:
            send_to_supplier_queue(deepcopy(MockFileDetails.flu_emis.sqs_message_body))
    ...
    def test_send_to_supplier_queue_failure_due_to_absent_supplier(self):
        ...
        with patch("send_sqs_message.sqs_client", mock_sqs_client):
            with self.assertRaises(InvalidSupplierError) as context:
                message_body = deepcopy(MockFileDetails.flu_emis.sqs_message_body)
    ...
    def test_make_and_send_sqs_message_success(self):
        ...
        self.assertIsNone(
            make_and_send_sqs_message(
                ...
                permission=deepcopy(MockFileDetails.flu_emis.permissions_list),
                vaccine_type=MockFileDetails.flu_emis.vaccine_type,
                supplier=MockFileDetails.flu_emis.supplier,
        ...
        messages = sqs_client.receive_message(QueueUrl=queue_url, MaxNumberOfMessages=1)
        self.assertEqual(
            json_loads(messages["Messages"][0]["Body"]), deepcopy(MockFileDetails.flu_emis.sqs_message_body)
        )
    ...
    def test_make_and_send_sqs_message_failure(self):
        ...
        with self.assertRaises(UnhandledSqsError) as context:
            make_and_send_sqs_message(
                file_key=MockFileDetails.flu_emis,
                message_id=MockFileDetails.flu_emis.message_id,
                permission=deepcopy(MockFileDetails.flu_emis.permissions_list),
                vaccine_type=MockFileDetails.flu_emis.vaccine_type,
                supplier=MockFileDetails.flu_emis.supplier,
